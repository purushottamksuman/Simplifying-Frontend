/*
  ...
  3. Constraints
    ...
    - Total time and maximum marks must be positive
    - Age range validation (max >= min when both provided)
*/
...
CHECK (length(trim(assessment_name)) > 0);

ALTER TABLE assessments 
ADD CONSTRAINT assessments_positive_time 
CHECK (total_time > 0);

ALTER TABLE assessments 
ADD CONSTRAINT assessments_positive_marks 
CHECK (maximum_marks > 0);

ALTER TABLE assessments 
ADD CONSTRAINT assessments_valid_age_range 
CHECK (
